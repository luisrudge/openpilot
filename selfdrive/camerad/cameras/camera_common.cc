...
void CameraBuf::send_yuv(AImage *image, uint32_t frame_id, const FrameMetadata& meta_data) {
  ...
  assert(uvPixelStride == 2);
  assert(uPixel == vPixel + 1);
...
//    rgb_width, (srcRect.right - srcRect.left), rgb_height, (srcRect.bottom - srcRect.top), yStride, uvStride, yPixel, yLen, cur_yuv_buf);
  uint8_t *dest = (uint8_t *)cur_yuv_buf->addr;
  for (int32_t y = 0; y < height; y++) {
    const uint8_t *pY = yPixel + yStride * (y + srcRect.top) + srcRect.left;
    int32_t uv_row_start = uvStride * ((y + srcRect.top) >> 1);
    const uint8_t *pUV = vPixel + uv_row_start + (srcRect.left >> 1);
